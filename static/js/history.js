...
function generateStatsFromPredictions(predictions) {
    ...
    const validConfidences = predictions.filter(p => p.confidence != null && !isNaN(p.confidence) && p.confidence >= 0);
    ...
    const lastBMI = latest.bmi || calculateBMI(latest.weight, latest.height);
...
function updateStats(stats) {
    ...
    const confidencePercent = (stats.average_confidence != null && !isNaN(stats.average_confidence)) 
        ? (stats.average_confidence * 100).toFixed(0) + '%' 
        : 'N/A';
    document.getElementById('avgConfidence').textContent = confidencePercent;
...
function displayPredictions() {
    ...
    const predictionsHTML = filteredPredictions.map(prediction => {
        ...
        const bmi = prediction.bmi || calculateBMI(prediction.weight, prediction.height);
            ...
            <div class="prediction-item" onclick="showPredictionDetails(${prediction.id})">
                <div class="d-flex justify-content-between align-items-start">
                    ...
                    <div class="text-end">
                        ...
                        <div class="fw-bold">${prediction.confidence != null && !isNaN(prediction.confidence) ? (prediction.confidence * 100).toFixed(0) + '%' : 'N/A'}</div>
                        <div class="confidence-bar" style="width: 80px;">
                            <div class="confidence-fill" style="width: ${prediction.confidence != null && !isNaN(prediction.confidence) ? prediction.confidence * 100 : 0}%"></div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                ...
                <div class="text-center">
                    ...
                    <div>Confiance: <strong>${prediction.confidence != null && !isNaN(prediction.confidence) ? (prediction.confidence * 100).toFixed(1) + '%' : 'N/A'}</strong></div>
                </div>
            </div>
        </div>
